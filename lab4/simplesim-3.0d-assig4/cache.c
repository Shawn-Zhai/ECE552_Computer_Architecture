...
void next_line_prefetcher(struct cache_t *cp, md_addr_t addr) {
  ...
  md_addr_t next_block_addr = addr + cp->bsize;
  next_block_addr = CACHE_BADDR(cp, next_block_addr);
...
#define STRIDE_RPT_SIZE 16
#define STRIDE_RPT_INDEX_BITS 4

struct stride_rpt_entry_t {
  md_addr_t tag;       
  md_addr_t prev_addr; 
  int stride;          
  int state; // state: 0 = initial, 1 = transient, 2 = steady, 3 = no prediction
};

struct stride_rpt_entry_t stride_rpt[STRIDE_RPT_SIZE];

/* Stride Prefetcher */
void stride_prefetcher(struct cache_t *cp, md_addr_t addr) {
	md_addr_t pc = get_PC() >> 3;
  int index = pc & (STRIDE_RPT_SIZE - 1);
  md_addr_t tag = pc >> STRIDE_RPT_INDEX_BITS;

  if (stride_rpt[index].tag != tag) {
    stride_rpt[index].tag = tag;
    stride_rpt[index].prev_addr = addr;
    stride_rpt[index].stride = 0;
    stride_rpt[index].state = 0;
  }

  else {
    int new_stride = addr - stride_rpt[index].prev_addr;

    switch (stride_rpt[index].state) {
      case 0: // initial
        if (new_stride == stride_rpt[index].stride) {
          stride_rpt[index].state = 2; // move to steady
        } 
        else {
          stride_rpt[index].state = 1; // move to transient
          stride_rpt[index].stride = new_stride;
        }
        break;
      ...
      case 1: // transient
        if (new_stride == stride_rpt[index].stride) {
          ...
          stride_rpt[index].state = 3; // move to no pred
          stride_rpt[index].stride = new_stride;
        }
        break;
      ...
      case 2: // steady
        if (new_stride != stride_rpt[index].stride) {
          stride_rpt[index].state = 0; // move to init
        }
        break;
  ...
  if (stride_rpt[index].state != 3) {
    md_addr_t prefetch_addr = addr + stride_rpt[index].stride;
...
#define OPEN_ENDED_RPT_SIZE 32
#define OPEN_ENDED_RPT_INDEX_BITS 5
#define PREFETCH_DEPTH 2
#define CONFIDENCE_THRESHOLD 2

struct open_ended_rpt_entry_t {
  md_addr_t tag;
  md_addr_t prev_addr;
  int stride;
  int state;  // 0 = initial, 1 = transient, 2 = steady, 3 = no prediction
  int confidence; // confidence counter for the stride
};

struct open_ended_rpt_entry_t open_ended_rpt[OPEN_ENDED_RPT_SIZE];

/* Open Ended Prefetcher */
void open_ended_prefetcher(struct cache_t *cp, md_addr_t addr) {
  md_addr_t pc = get_PC() >> 3;
  int index = pc & (OPEN_ENDED_RPT_SIZE - 1);
  md_addr_t tag = pc >> OPEN_ENDED_RPT_INDEX_BITS;

  if (open_ended_rpt[index].tag != tag) {
    open_ended_rpt[index].tag = tag;
    open_ended_rpt[index].prev_addr = addr;
    open_ended_rpt[index].stride = 0;
    open_ended_rpt[index].state = 0;
    open_ended_rpt[index].confidence = 0;
    return;
  }

  int new_stride = addr - open_ended_rpt[index].prev_addr;

  switch (open_ended_rpt[index].state) {
    case 0: // initial
      if (new_stride == open_ended_rpt[index].stride) {
        open_ended_rpt[index].confidence++;
        if (open_ended_rpt[index].confidence >= CONFIDENCE_THRESHOLD) {
          open_ended_rpt[index].state = 2; // move to steady
        }
      } 
      else {
        open_ended_rpt[index].state = 1; // move to transient
        open_ended_rpt[index].stride = new_stride;
        open_ended_rpt[index].confidence = 1;
      }
      break;
    ...
    case 1: // transient
      if (new_stride == open_ended_rpt[index].stride) {
        ...
        open_ended_rpt[index].state = 3; // move to no prediction
        open_ended_rpt[index].confidence = 0;
      }
      break;
    ...
    case 2: // Steady
      if (new_stride != open_ended_rpt[index].stride) {
        open_ended_rpt[index].state = 0; // move to init
        open_ended_rpt[index].confidence = 0;
      }
      break;
  ...
  // prefetch only in steady state or high confidence
  if (open_ended_rpt[index].state == 2 || (open_ended_rpt[index].state == 1 && open_ended_rpt[index].confidence >= CONFIDENCE_THRESHOLD)) {
    for (int i = 1; i <= PREFETCH_DEPTH; i++) {
      md_addr_t prefetch_addr = addr + i * open_ended_rpt[index].stride;
      prefetch_addr = CACHE_BADDR(cp, prefetch_addr);
